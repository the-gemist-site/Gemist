(function () {
  ...
  // ---------------------------------------------------------
  // Image zoom — lets a visitor magnify any photo (mainly to
  // examine a stone closely) in its own overlay, so the page
  // itself never has to scroll or resize to allow it. Scroll
  // wheel / pinch to zoom, drag to pan once zoomed in.
  // ---------------------------------------------------------
  var imageZoom = document.getElementById("imageZoom");
  var imageZoomImg = document.getElementById("imageZoomImg");
  var imageZoomClose = document.getElementById("imageZoomClose");
  var zoomScale = 1;
  var zoomPanX = 0;
  var zoomPanY = 0;
  var zoomPointers = new Map();
  var zoomPinchStartDist = null;
  var zoomPinchStartScale = null;
  var zoomDragStart = null;
  var zoomDragStartPan = null;

  function clampZoomScale(z) {
    return Math.min(4, Math.max(1, z));
  }

  function zoomFitSize() {
    var vw = window.innerWidth;
    var vh = window.innerHeight;
    var ratio = (imageZoomImg.naturalWidth && imageZoomImg.naturalHeight)
      ? imageZoomImg.naturalWidth / imageZoomImg.naturalHeight
      : vw / vh;
    var boxRatio = vw / vh;
    if (ratio > boxRatio) return { w: vw, h: vw / ratio };
    return { w: vh * ratio, h: vh };
  }

  function clampZoomPan(scale, x, y) {
    var fit = zoomFitSize();
    var maxX = Math.max(0, (fit.w * scale - fit.w) / 2);
    var maxY = Math.max(0, (fit.h * scale - fit.h) / 2);
    return {
  ...
  function applyImageZoomTransform() {
    imageZoomImg.style.transform = "translate(" + zoomPanX + "px, " + zoomPanY + "px) scale(" + zoomScale + ")";
  }
  ...
  function openImageZoom(src, alt) {
    if (!imageZoom || !imageZoomImg || !src) return;
  ...
  function closeImageZoom() {
    if (imageZoom) imageZoom.classList.remove("is-open");
  }
  ...
  document.querySelectorAll(".zoom-trigger").forEach(function (btn) {
    btn.addEventListener("click", function (e) {
      e.preventDefault();
      e.stopPropagation();
      var target = btn.dataset.zoomTarget;
      var img = target === "landing"
        ? document.querySelector(".landing-img")
        : document.querySelector(".piece-img.is-active");
  ...
  if (imageZoom) {
    imageZoom.addEventListener("click", function (e) {
      if (e.target === imageZoom || e.target.classList.contains("image-zoom-viewport")) closeImageZoom();
    });
  }
  window.addEventListener("keydown", function (e) {
    if (e.key === "Escape" && imageZoom && imageZoom.classList.contains("is-open")) closeImageZoom();
  });

  if (imageZoomImg) {
    imageZoomImg.addEventListener("wheel", function (e) {
      e.preventDefault();
      var delta = -e.deltaY * 0.002;
      zoomScale = clampZoomScale(zoomScale * (1 + delta));
      var clamped = clampZoomPan(zoomScale, zoomPanX, zoomPanY);
      zoomPanX = clamped.x;
      zoomPanY = clamped.y;
      applyImageZoomTransform();
    }, { passive: false });

    imageZoomImg.addEventListener("dblclick", function () {
      if (zoomScale > 1) {
        resetImageZoom();
      } else {
        zoomScale = 2.5;
    ...
    imageZoomImg.addEventListener("pointerdown", function (e) {
      imageZoomImg.setPointerCapture(e.pointerId);
      zoomPointers.set(e.pointerId, { x: e.clientX, y: e.clientY });
      if (zoomPointers.size === 1) {
        zoomDragStart = { x: e.clientX, y: e.clientY };
        zoomDragStartPan = { x: zoomPanX, y: zoomPanY };
    ...
    imageZoomImg.addEventListener("pointermove", function (e) {
      if (!zoomPointers.has(e.pointerId)) return;
      zoomPointers.set(e.pointerId, { x: e.clientX, y: e.clientY });
      if (zoomPointers.size === 2 && zoomPinchStartDist) {
        var pts = Array.from(zoomPointers.values());
        var dist = Math.hypot(pts[0].x - pts[1].x, pts[0].y - pts[1].y);
        zoomScale = clampZoomScale(zoomPinchStartScale * (dist / zoomPinchStartDist));
        var clamped = clampZoomPan(zoomScale, zoomPanX, zoomPanY);
        zoomPanX = clamped.x;
        zoomPanY = clamped.y;
        applyImageZoomTransform();
      } else if (zoomPointers.size === 1 && zoomScale > 1 && zoomDragStart) {
        var dx = e.clientX - zoomDragStart.x;
        var dy = e.clientY - zoomDragStart.y;
        var clampedDrag = clampZoomPan(zoomScale, zoomDragStartPan.x + dx, zoomDragStartPan.y + dy);
        zoomPanX = clampedDrag.x;
        zoomPanY = clampedDrag.y;
        applyImageZoomTransform();
      }
    });
    ...
    function endZoomPointer(e) {
      ...
      if (zoomPointers.size < 2) zoomPinchStartDist = null;
      if (zoomPointers.size === 0) zoomDragStart = null;
    }
    imageZoomImg.addEventListener("pointerup", endZoomPointer);
    imageZoomImg.addEventListener("pointercancel", endZoomPointer);
  }
